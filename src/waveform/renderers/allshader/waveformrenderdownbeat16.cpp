...
#include "moc_waveformrenderdownbeat16.cpp"
...
WaveformRenderDownBeat16::WaveformRenderDownBeat16(WaveformWidgetRenderer* waveformWidget,
...
void WaveformRenderDownBeat16::setup(const QDomNode& node, const SkinContext& skinContext) {
    m_color = QColor(skinContext.selectString(node, QStringLiteral("DownbeatColor2")));
    m_color = WSkinColor::getCorrectColor(m_color).toRgb();
}

void WaveformRenderDownBeat16::draw(QPainter* painter, QPaintEvent* event) {
    Q_UNUSED(painter);
    Q_UNUSED(event);
    DEBUG_ASSERT(false);
}

void WaveformRenderDownBeat16::preprocess() {
    if (!preprocessInner()) {
        geometry().allocate(0);
        markDirtyGeometry();
    }
...
bool WaveformRenderDownBeat16::preprocessInner() {
    ...
    for (auto it = trackBeats->iteratorFrom(startPosition);
            ...
            ++it) {
        if ((it.beatOffset() - downbeatOffset) % 16 != 0) {
            continue;
        }
        numDownbeatsInRange++;
    }
